...
close_prices = forex_data['value'].to_numpy()
...
filtered_frequencies = positive_frequencies[(positive_frequencies >= min_frequency_threshold) & (
    positive_frequencies <= frequency_range[1])]
filtered_amplitudes = positive_fft_values[(positive_frequencies >= min_frequency_threshold) & (
    positive_frequencies <= frequency_range[1])]

# Identifying significant frequencies above the amplitude threshold
significant_peaks, _ = find_peaks(
    filtered_amplitudes, height=amplitude_threshold)
...
plt.plot(filtered_frequencies, filtered_amplitudes,
         label='Filtered FFT Amplitudes')
plt.plot(significant_frequencies, significant_amplitudes,
         'r*', label='Significant Frequencies')
...
plt.plot(significant_days_per_cycle, significant_amplitudes,
         'r*', label='Significant Cycles')

...
class TradingStrategy:
    def __init__(self, model, data, start_cash=10000, trading_lot=2500, stop_loss_threshold=0.05, take_profit_threshold=0.05, leverage_factor=1):
        self.model = model
        self.data = data
    ...
    def execute_trades(self):
        ...
        for index, (row, prediction) in enumerate(zip(self.data.iterrows(), predicted_categories)):
            ...
            if prediction == 'Buy' and self.cash >= self.trading_lot and ( self.buy_price is None or (self.buy_price is not None and ( usd_jpy_spot_rate < self.buy_price * 0.99 or usd_jpy_spot_rate > self.buy_price * 1.01) ) ):
            ...
            elif prediction == 'Sell' and self.jpy_inventory > 0:
